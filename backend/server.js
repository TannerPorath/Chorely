...
// ============ MIGRATION: add description column to chores if missing ============
const hasChoreDescriptionCol = db.prepare("PRAGMA table_info(chores)").all()
  .some(c => c.name === 'description');
if (!hasChoreDescriptionCol) {
  console.log('Adding description column to chores...');
  db.exec("ALTER TABLE chores ADD COLUMN description TEXT");
}
...
app.post('/api/chores', (req, res) => {
  const { title, icon, value, assignedTo, frequency, isRequiredForExtras, maxClaimers, description } = req.body;
  ...
  const desc = description ? String(description).slice(0, 200) : null;
  // assignedTo === null means it's an Extra Chore (shared pool, anyone can claim)
  const id = genId('c');
  db.prepare('INSERT INTO chores (id, title, icon, value, assigned_to, frequency, is_required_for_extras, max_claimers, description) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(
    id, title, icon, parseFloat(value), assignedTo || null, freq, isRequiredForExtras ? 1 : 0, mc, desc
  );
...
app.put('/api/chores/:id', (req, res) => {
  ...
  const desc = description !== undefined ? (description ? String(description).slice(0, 200) : null) : existing.description;
  db.prepare('UPDATE chores SET title=?, icon=?, value=?, assigned_to=?, frequency=?, is_required_for_extras=?, max_claimers=?, description=? WHERE id=?').run(
